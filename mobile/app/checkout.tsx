...
export default function CheckoutScreen() {
  ...
  const complete = async () => {
    ...
    if (!address.trim()) return showToast('Escribe la dirección de entrega.', 'error')
    setSaving(true)
    try { const order = await api.checkout({ cart_id: cart.id, shipping_address: address.trim(), payment_method: payment, idempotencyKey: `mobile-${Date.now()}` }); await refresh(); showToast(`Tu pedido #${order.id} fue creado correctamente.`, 'success'); router.replace('/(tabs)/account') }
    catch { showToast('No fue posible completar el pedido. Revisa la información e inténtalo nuevamente.', 'error') }
